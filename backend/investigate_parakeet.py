...
for attr in ['text', 'segments', 'words', 'sentences', 'tokens', 'language']:
    if hasattr(result, attr):
        ...
        if isinstance(val, list) and len(val) > 0:
            ...
            print(f"  First item attributes: {[a for a in dir(val[0]) if not a.startswith('_')][:10]}")
    else:
...
if hasattr(result, 'sentences'):
    ...
    if len(sentences) > 0:
        ...
        print(f"First sentence attributes: {[a for a in dir(first_sentence) if not a.startswith('_')]}")
        ...
        if hasattr(first_sentence, 'tokens'):
            ...
            if len(tokens) > 0:
                ...
                print(f"  First token attributes: {[a for a in dir(first_token) if not a.startswith('_')]}")
