...
def test_resolution_clipping():
    ...
    for res_name, (video_width, video_height) in resolutions.items():
        ...
        print(
            f"Calculated font size: {calculated_font_size}px (from base {base_font_size}px)"
        )
        ...
        draw.line(
            [(0, bottom_line_y), (new_width, bottom_line_y)], fill="yellow", width=2
        )
...
def test_1080p_with_fixes():
    ...
    for test_case in test_cases:
        ...
        print(
            f"  Y-position: {vertical_position}px (center at {test_case['y_percent']*100}%)"
        )
        print(f"  Text bottom: {text_bottom}px")
        print(f"  Clearance: {clearance}px", end="")
